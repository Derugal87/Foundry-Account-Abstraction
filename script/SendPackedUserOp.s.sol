...
contract SendPackedUserOp is Script {
    ...
    function run() external {
        ...
        address dest = config.getConfig().usdc; // arbitrum mainnet USDC address
        uint256 value = 0;
        ...
        PackedUserOperation memory userOp =
            generateSignedUserOp(executeCalldata, config.getConfig(), minimalAccountAddress);
    ...
    function generateSignedUserOp(
        ...
        // 3. Sign it, and return the signed data
        uint8 v;
        bytes32 r;
        bytes32 s;
        ...
        userOp.signature = abi.encodePacked(r, s, v); // r, s, v order is important, so note it
        return userOp;
    }
